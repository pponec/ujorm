 ...
 * @author Pavel Ponec
 */
@WebServlet(BenchmarkStockBuild.URL_PATTERN)
public class BenchmarkStockBuild extends HttpServlet {

    /** URL pattern */
    public static final String URL_PATTERN = "/benchmarkStockBulid";
    ...
    public void stock(HttpServletResponse output) throws IOException, IllegalArgumentException {
        ...
        try (XmlBuilder html = new XmlBuilder(Html.HTML, writer)) {
            ...
            try (XmlBuilder body = html.addElement(Html.BODY)) {
                ...
                try (XmlBuilder table = body.addElement(Html.TABLE)) {
                    try (XmlBuilder thead = table.addElement(Html.THEAD)) {
                    ...
                    for (int itemIndex = 0, max = stocks.size(); itemIndex < max; itemIndex++) {
                        ...
                        try (XmlBuilder row = table.addElement(Html.TR, Html.A_CLASS, itemIndex % 2 == 0 ? "odd" : "even")) {
        ...
        output.getWriter().append(writer.toString());
    }
