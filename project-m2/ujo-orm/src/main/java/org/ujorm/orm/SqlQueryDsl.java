 ...
 * @since 2.26
 */
public class SqlQueryDsl<D extends SqlQueryDsl<D>> extends AbstractSqlQuery<SqlQueryDsl<D>> {

    /**
     * Constructor with a database connection
     * @param dbConnection A database connection
     */
    public SqlQueryDsl(@NotNull Connection dbConnection) {
        super(dbConnection);
    }

    public <V> SqlQueryDsl<D> select (Key<D,V> key, CharSequence... keys) {
        return this;
    }
    ...
    public static <R> R run(Connection connection, final SqlFunction<SqlQueryDsl, R> fun) {
        try (var query = new SqlQueryDsl(connection)) {
