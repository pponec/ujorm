...
public class MultiRun<P> {
    ...
    protected <R> Function<CompletableFuture<R>, R> createFunction(@Nonnull final Duration timeout) {
        return (Grabber<R>) (CompletableFuture<R> t) -> t.get(timeout.toMillis(), MILLISECONDS);
    }
    ...
    /** Grab a result value from a CompletableFuture object */
    protected interface Grabber<R> extends Function<CompletableFuture<R>, R> {

        @Override
        default R apply(@Nonnull final CompletableFuture<R> t) {
            try {
                return convert(t);
            } catch (InterruptedException | ExecutionException | TimeoutException ex) {
                throw new MultiRunException(ex);
            }
        }

        R convert(CompletableFuture<R> t) throws InterruptedException, ExecutionException, TimeoutException;
    }
