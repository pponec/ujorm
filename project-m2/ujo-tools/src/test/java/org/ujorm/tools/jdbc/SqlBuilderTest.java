...
public class SqlBuilderTest extends AbstractJdbcConnector {
    ...
    void regexpTest() {
        // Test pro :hello
        var matcher = SqlBuilder.SQL_MARK.matcher(":hello");
        assertEquals("hello", matcher.find() ? matcher.group(1) : "");

        // Test pro :abc123
        matcher = SqlBuilder.SQL_MARK.matcher(":abc123");
        assertEquals("abc123", matcher.find() ? matcher.group(1) : "");

        // Test pro :test_
        matcher = SqlBuilder.SQL_MARK.matcher(":test_");
        assertEquals("test_", matcher.find() ? matcher.group(1) : "");

        // Test pro :test%
        matcher = SqlBuilder.SQL_MARK.matcher(":test%");
        assertEquals("test", matcher.find() ? matcher.group(1) : "");

        // Test pro text bez shody
        matcher = SqlBuilder.SQL_MARK.matcher("hello");
        assertEquals("", matcher.find() ? matcher.group(1) : "");

        // Test pro text s dvojtečkou, ale bez \w+
        matcher = SqlBuilder.SQL_MARK.matcher(":");
        assertEquals("", matcher.find() ? matcher.group(1) : "");
    }
    ...
    private void newDemo(Connection dbConnection) {
        List<Employee> employees = SqlBuilder.run(dbConnection, build -> build.sql("""
    ...
    private void runSqlStatementsLike(SqlBuilder query) {
    ...
    void testSqlExceptionWrapping() throws SQLException {
        ...
        try (var query = new SqlBuilder(dbConnection)) {
            var ex = assertThrows(SqlBuilder.SqlException.class, () -> {
