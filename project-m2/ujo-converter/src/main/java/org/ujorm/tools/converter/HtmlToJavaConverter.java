...
/** The class was generated by Gemini Pro. */
...
public final class HtmlToJavaConverter {
    ...
    private void init() {
        ...
        for (var field : Html.class.getDeclaredFields()) {
        ...
        var elementClass = org.ujorm.tools.web.Element.class;
        for (var method : elementClass.getMethods()) {
            ...
            if (name.startsWith("add") && name.length() > 3) {
                var rawSuffix = name.substring(3).toLowerCase();
                // Apply overrides from the Map (e.g. "listitem" -> "li")
                var tagName = TAG_OVERRIDES.getOrDefault(rawSuffix, rawSuffix);
    ...
    private void writeRecursive(Node node,
        ...
        if (node instanceof Element element) {
            ...
            var useBlock = blockStyle || hasElementChildren;
    ...
    private void writeTextCommon(Appendable writer, String indent, String prefix, String suffix, String method, String text) throws IOException {
        ...
        var callPrefix = prefix.startsWith("\n") ? prefix : indent + prefix;
    ...
    private void printTextCall(Appendable writer, String prefix, String method, String text, String indent) throws IOException {
        ...
        if (text.contains("\n")) {
            var blockIndent = indent + OFFSET;
            writeMultilineContent(writer, text, blockIndent);
        } else {
            var nextLineIndent = indent + OFFSET + (prefix.startsWith("\n") ? "" : OFFSET);
            // Single line string
            writer.append("\"")
    ...
    private void writeMultilineContent(Appendable writer, String input, String blockIndent) throws IOException {
        ...
        var body = text.substring(leadingNewLines);
    ...
    private @NotNull String getAndRemoveTitle(Document document) {
    ...
    private String generateVariableName(String tagName, Deque<String> ancestors) {
        ...
        var baseName = lowerTag.contains("-") ? toCamelCase(lowerTag) : lowerTag;

        if (HEADING_PATTERN.matcher(baseName).matches()) {
            return baseName;
        }

        var count = ancestors.stream()
    ...
    private void writeAttributes(Element element, Appendable writer, String prefix, String suffix, Set<String> ignoreKeys, boolean separatedCssStyles, Map<String, String> cssConstants) throws IOException {
        ...
        for (var attr : element.attributes()) {
            ...
            if (setterName != null) {
                ...
                var attrCode = htmlConstants.getOrDefault(key, "\"" + key + "\"");
                writer.append(prefix)
                        .append(".setAttribute(")
                        .append(attrCode)
                        .append(", \"")
    ...
    private String escapeJavaTextBlock(String input, String indent) {
        ...
        var result = indent + sb.toString().replace("\n", "\n" + indent);
