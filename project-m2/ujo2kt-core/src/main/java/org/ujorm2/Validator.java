...
import org.ujorm2.validator.impl.ValidatorFactory;
...
public interface Validator<VALUE> {
     ...
     */
    public <UJO> ValidationError validate(Key<UJO, VALUE> key, UJO bo);
     ...
     */
    public <UJO> void checkValue(VALUE value, Key<UJO, VALUE> key, UJO bo) throws ValidationException;

    /** Throw an exception if input value is not valid.
     * @param key UJO Key
     * @param bo Target Domain object @null value} if the result is ok.
     */
    public <UJO> void checkValue(Key<UJO, VALUE> key, UJO bo) throws ValidationException;
