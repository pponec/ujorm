...
public class KeyColumnLocalDate<U extends Ujo> extends KeyColumn<U, LocalDate> {
    ...
    public KeyColumnLocalDate(KeyRing<U> key, KeyRing<U> keySortable) {
        this(key, keySortable, DateTimes.LOCALE_DATE_FORMAT_KEY);
    }

    public KeyColumnLocalDate(KeyRing<U> key, KeyRing<U> keySortable, String localeDateFormatKey) {
    ...
    protected Component createValueCoponent(final String componentId, final IModel<?> valueModel, final U ujo) {
        ...
        result.setDefaultModel(new Model() {
            @Override public Serializable getObject() {
                final LocalDate modelObject = (LocalDate) valueModel.getObject();
                if (modelObject != null) {
    ...
    public static <U extends Ujo, T> KeyColumn<U,T> of(Key<U,T> key, boolean sorted, String cssClass, String localeDateFormatKey) {
        ...
        final KeyColumn<U, T> result = new KeyColumnLocalDate
                ( serializableKey
